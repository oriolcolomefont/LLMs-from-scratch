...
class SpamDataset(Dataset):
    def __init__(
        self, csv_file, tokenizer, max_length=None, pad_token_id=50256, no_padding=False
    ):
...
def calc_loss_batch(
    ...
    if (
    ...
    ):  # Selects the last tokens before the padding tokens
        ...
        loss = torch.nn.functional.cross_entropy(
            logits, target_batch, ignore_index=ignore_index
        )
        return loss
...
def train_classifier_simple(
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (input_batch, target_batch) in enumerate(train_loader):
            ...
            # Use gradient accumulation if accumulation_steps > 1
            is_update_step = ((batch_idx + 1) % accumulation_steps == 0) or (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch_size", type=int, default=8, help=("The batch size used for training.")
    )
    parser.add_argument(
        "--accumulation_steps",
        type=int,
    ...
    if args.trainable_layers == "last_layer":
        ...
        args.trainable_layers == "last_block"
        or args.trainable_layers == "last_two_blocks"
    ):
        ...
        replace_linear_with_lora(
            model, rank=args.lora_rank, alpha=args.lora_alpha, alternative=alternative
        )
    else:
